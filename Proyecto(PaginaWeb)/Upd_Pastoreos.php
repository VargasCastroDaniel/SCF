...
<body>
    <?php
        ...
        $NumeroLoteNuevo = $_GET['NumeroLoteNuevo'];
        ...
        $tsql= "execute SCF.upd_Pastoreos '$NumeroLote', '$NumeroLoteNuevo', '$CantidadReses', '$FechaHoraInicio', '$FechaHoraFinal'";
        $stmt = sqlsrv_prepare($conn, $tsql);
        $result = sqlsrv_execute($stmt);
        if($stmt){
            $error = sqlsrv_errors();
            $key = array_values($error[0]);
            $result2 = substr($key[4], -22);
            if ($result2 == "NumeroLote Inexistente"){
                echo($result2);
                echo($espacio);
            }else{
    ...
    <div class="Atras">
            <input type="button" value="Atras" onclick="window.location='Upd_Pastoreos.html'">
    </div>
</body>
</html>