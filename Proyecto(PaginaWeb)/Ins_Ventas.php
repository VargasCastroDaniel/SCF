...
<body>
    <?php
        ...
        $tsql= "execute SCF.ins_Ventas '$Fecha', '$TotalLitros'";
        $stmt = sqlsrv_prepare($conn, $tsql);
        $result = sqlsrv_execute($stmt);
        if($stmt){
            $error = sqlsrv_errors();
            $key = array_values($error[0]);
            $result2 = substr($key[4], -14);
            if ($result2 == "Fecha Repetida"){
                echo($result2);
                echo($espacio);
            }else{
                $result3 = substr($key[4], -35);
                echo($result3);
            }
        }
    ?>
    ...
    <div class="Atras">
            <input type="button" value="Atras" onclick="window.location='Ins_Ventas.html'">
    </div>
</body>
</html>