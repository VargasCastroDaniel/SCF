...
<head>
    ...
    <title>Sel_V_Medicamentos</title>
...
<body>
    <?php
        ...
        $tsql= "execute SCF.sel_VacunacionesMedicamentos '$Nombre'";
        $getResults= sqlsrv_query($conn, $tsql);
        if($getResults){
            while ($row = sqlsrv_fetch_array($getResults, SQLSRV_FETCH_ASSOC)) 
            {
                echo ("Nombre: $row[Nombre]<hr>");
                echo ("Fecha y Hora de la vacunacion: $row[FechayHoradelavacunacion]<hr>");
                echo ("Dosis: $row[Dosis]<hr>");
            }
        }else{
    ...
    <div class="Atras">
            <input type="button" value="Atras" onclick="window.location='Sel_V_Medicamentos.html'">
    </div>
</body>
</html>