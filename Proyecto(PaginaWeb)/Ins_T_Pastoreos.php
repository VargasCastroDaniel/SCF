...
<body>
    <?php
        ...
        $tsql= "execute SCF.ins_TrabajadoresPastoreos '$Cedula', '$NumeroLote'";
        $stmt = sqlsrv_prepare($conn, $tsql);
        $result = sqlsrv_execute($stmt);
        if($stmt){
            $error = sqlsrv_errors();
            $key = array_values($error[0]);
            $result2 = substr($key[4], -54);
            if ($result2 == "La cedula del trabajador o el numero de lote no existe"){
                echo($result2);
                echo($espacio);
            }else{
                $result3 = substr($key[4], -50);
                echo($result3);
            }
        }
    ?>
    ...
    <div class="Atras">
            <input type="button" value="Atras" onclick="window.location='Ins_T_Pastoreos.html'">
    </div>
</body>
</html>