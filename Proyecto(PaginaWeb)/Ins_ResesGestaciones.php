...
<head>
    ...
    <title>Ins_ResesOvulaciones</title>
...
<body>
    <?php
        ...
        $tsql= "execute SCF.ins_ResesGestaciones '$IdRes', '$FechaHoraPreñez'";
        $stmt = sqlsrv_prepare($conn, $tsql);
        $result = sqlsrv_execute($stmt);
        if($stmt){
            $error = sqlsrv_errors();
            $key = array_values($error[0]);
            $result2 = substr($key[4], -52);
            if ($result2 == "El Id de la res o La fecha de la gestacion no existe"){
                echo($result2);
                echo($espacio);
            }else{
    ...
    <div class="Atras">
            <input type="button" value="Atras" onclick="window.location='Ins_ResesGestaciones.html'">
    </div>
</body>
</html>