...
<body>
    <?php
        ...
        $tsql= "execute SCF.ins_ResesOrdeños '$IdRes', '$FechaHora'";
        $stmt = sqlsrv_prepare($conn, $tsql);
        $result = sqlsrv_execute($stmt);
        if($stmt){
            $error = sqlsrv_errors();
            $key = array_values($error[0]);
            $result2 = substr($key[4], -47);
            if ($result2 == "El Id de la res o La fecha del ordeño no existe"){
                echo($result2);
                echo($espacio);
            }else{
    ...
    <div class="Atras">
            <input type="button" value="Atras" onclick="window.location='Ins_ResesOrdeños.html'">
    </div>
</body>
</html>