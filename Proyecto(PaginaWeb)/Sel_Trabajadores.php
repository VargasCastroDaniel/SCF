...
<body>
    <?php
        ...
        $tsql= "execute SCF.sel_Trabajadores '$cedula'";
        $getResults= sqlsrv_query($conn, $tsql);
        if($getResults){
            while ($row = sqlsrv_fetch_array($getResults, SQLSRV_FETCH_ASSOC)) 
            {
                echo ("Nombre del trabajador: $row[NombreCompleto]<hr>");
                echo ("Cedula: $row[Cedula]<hr>");
                echo ("Telefono: $row[Telefono]<hr>");
            }
        }else{
    ...
    <div class="Atras">
            <input type="button" value="Atras" onclick="window.location='Sel_Trabajadores.html'">
    </div>
</body>
</html>
