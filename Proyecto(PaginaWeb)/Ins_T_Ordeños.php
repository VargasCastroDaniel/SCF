...
<head>
    ...
    <title>Ins_T_Ordeños</title>
    <link rel="stylesheet" href="./estilo_button.css">
</head>
<body>
    <?php
        $Cedula = $_GET['Cedula'];
        $FH_Ordeños = $_GET['FH_Ordeños'];
        ...
        $tsql= "execute SCF.ins_TrabajadoresOrdeños '$Cedula', '$FH_Ordeños'";
        $stmt = sqlsrv_prepare($conn, $tsql);
        $result = sqlsrv_execute($stmt);
        if($stmt){
            $error = sqlsrv_errors();
            $key = array_values($error[0]);
            $result2 = substr($key[4], -56);
            if ($result2 == "La cedula del trabajador o la fecha del ordeño no existe"){
                echo($result2);
                echo($espacio);
            }else{
                $result3 = substr($key[4], -49);
                echo($result3);
            }
        }
    ?>
    ...
    <div class="Atras">
            <input type="button" value="Atras" onclick="window.location='Ins_T_Ordeños.html'">
    </div>
</body>
</html>