...
<head>
    ...
    <title>Ins_ResesVacunaciones</title>
...
<body>
    <?php
        ...
        $tsql= "execute SCF.ins_ResesVacunaciones '$IdRes', '$FechaHora'";
        $stmt = sqlsrv_prepare($conn, $tsql);
        $result = sqlsrv_execute($stmt);
        if($stmt){
            $error = sqlsrv_errors();
            $key = array_values($error[0]);
            $result2 = substr($key[4], -53);
            if ($result2 == "El Id de la res o la fecha de la vacunacion no existe"){
                echo($result2);
                echo($espacio);
            }else{
                $result3 = substr($key[4], -46);
                echo($result3);
            }
        }
    ?>
    ...
    <div class="Atras">
            <input type="button" value="Atras" onclick="window.location='Ins_ResesVacunaciones.html'">
    </div>
</body>
</html>